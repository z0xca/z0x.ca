...
import { Replacer } from '@/components/Replacer.tsx'
...
Replace it with your domain here:

<Replacer string="domain.tld" label="Enter your domain:" placeholder="your-domain.tld" client:load>
...
You will need to configure your domain name registrar (I recommend using [Porkbun](https://porkbun.com)) to direct requests from `domain.tld` to your server's IP address.
Specifically, you'll need to create the following **A records**:
...
At this point, give the [Matrix Federation Tester](https://federationtester.matrix.org/) a whirl by entering `domain.tld`. This tool checks if your server is reachable on the expected ports. If you hit any issues, it's likely to do with the reverse proxy or delegation setup, assuming there's nothing wrong in the Synapse logs. Make sure to double check that your replaced every mention of `domain.tld` in the guide with your actual domain name and that your ports are actually open.

You should also be able to view the following at https://matrix.domain.tld/_matrix/static

<div class="max-w-md mx-auto flex justify-center prose-img:rounded-md dark:invert">
